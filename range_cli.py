...
def setup_vm_commands(subparsers):
    ...
    power_parser.add_argument('action', choices=['start', 'stop'], help='Action to perform')
    ...
    nuke_parser.add_argument('--no-stop', action='store_true', help="Don't stop VMs before deletion")
    nuke_parser.add_argument('--dry-run', action='store_true', help='Show what would be deleted without doing it')
    
    # Nuke gateway VMs specifically
    nuke_gw_parser = vm_subparsers.add_parser('nuke-gw', help='Delete all gateway VMs (ending in -range-gw)')
    nuke_gw_parser.add_argument('--dry-run', action='store_true', help='Show what would be deleted')
    ...
    clone_parser.add_argument('--full', action='store_true', help='Create full clone instead of linked')
...
def setup_network_commands(subparsers):
    ...
    net_parser = subparsers.add_parser('network', help='Network/VNet management operations')
    net_subparsers = net_parser.add_subparsers(dest='net_command', help='Network commands')
    ...
    # Ensure user VNet
    ensure_parser = net_subparsers.add_parser('ensure-user', help='Ensure VNet exists for user')
    ensure_parser.add_argument('username', help='Username to create VNet for')
    ensure_parser.add_argument('--zone', default='CMPCCDC', help='SDN zone (default: CMPCCDC)')
    
    # Ensure VNets for all AD users
    ensure_all_parser = net_subparsers.add_parser('ensure-all', help='Ensure VNets for all AD users')
    ensure_all_parser.add_argument('--zone', default='CMPCCDC', help='SDN zone (default: CMPCCDC)')
...
def setup_range_commands(subparsers):
    ...
    range_parser = subparsers.add_parser('range', help='High-level range setup operations')
    range_subparsers = range_parser.add_subparsers(dest='range_command', help='Range commands')
    
    # Setup range for user
    setup_parser = range_subparsers.add_parser('setup', help='Set up complete range for user')
    setup_parser.add_argument('username', help='Username (without realm)')
    setup_parser.add_argument('--base-vmid', type=int, default=150, help='Base VM to clone from (default: 150)')
    setup_parser.add_argument('--pool-suffix', default='-range', help='Pool name suffix (default: -range)')
    
    # Setup ranges for all AD users
    setup_all_parser = range_subparsers.add_parser('setup-all', help='Set up ranges for all AD users')
    setup_all_parser.add_argument('--base-vmid', type=int, default=150, help='Base VM to clone from (default: 150)')
    setup_all_parser.add_argument('--pool-suffix', default='-range', help='Pool name suffix (default: -range)')
    setup_all_parser.add_argument('--skip-admins', action='store_true', help='Skip users with -adm in name')
...
def handle_vm_commands(args, manager: RangeManager):
    ...
    if args.vm_command == 'list':
        ...
        print("-" * 60)
        for vm in vms:
            print(f"{vm['vmid']:<6} {vm.get('name', 'Unknown'):<30} {vm.get('status', 'Unknown'):<10} {vm.get('node', 'Unknown'):<10}")
        ...
        success = manager.vms.clone_vm(
            args.base_vmid, new_vmid, args.name, 
            args.pool, args.full
        )
        if success:
            print(f"Successfully cloned VM {args.base_vmid} to {new_vmid} ({args.name})")
        else:
...
def handle_network_commands(args, manager: RangeManager):
    ...
    if args.net_command == 'list':
        ...
        print("-" * 50)
        for vnet in vnets:
            print(f"{vnet.get('vnet', 'Unknown'):<15} {vnet.get('zone', 'Unknown'):<15} {vnet.get('alias', ''):<20}")
...
def handle_range_commands(args, manager: RangeManager):
    ...
    if args.range_command == 'setup':
        success = manager.setup_user_range(args.username, args.base_vmid, args.pool_suffix)
        if success:
        ...
        for user in ad_users:
            ...
            success = manager.setup_user_range(username, args.base_vmid, args.pool_suffix)
            if success:
